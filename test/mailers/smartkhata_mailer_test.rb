...
class SmartkhataMailerTest < ActionMailer::TestCase
  def setup
  ...
  test "bill email" do
    ...
    email = SmartkhataMailer.bill_email(@transaction_message.id, @tenant.id)
  ...
  test "transaction message email" do
    # Create the email and store it for further assertions
    email = SmartkhataMailer.transaction_message_email(@transaction_message.id, @tenant.id)
