...
RSpec.describe LedgerDaily, type: :model do
  ...
  describe "#sum_of_closing_balance_of_ledger_dailies_for_ledgers" do
    context "when last day ledger daily present" do
      ...
      subject{create(:ledger_daily, closing_balance: 1000, date: "2017-6-8" ,ledger: ledger, branch_id: @branch.id, fy_code: 7374)}

      it "should return sum of closing balance" do
        subject
        expect(LedgerDaily.sum_of_closing_balance_of_ledger_dailies_for_ledgers(ledger.id,subject.date)).to eq(1000)
      end
    end
    ...
    context "when last day daily ledger not present" do
      ...
      it "should return closing balance 0" do
        expect(LedgerDaily.sum_of_closing_balance_of_ledger_dailies_for_ledgers(ledger.id,"2017-6-8")).to eq(0)
      end
    end
  end
