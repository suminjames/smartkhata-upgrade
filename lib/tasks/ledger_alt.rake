namespace :ledger_alt do
  def patch_ledger_dailies(ledger, all_fiscal_years = false, branch_id = 1)
    ...
    fy_codes.each do |fy_code|
      ...
      transaction_dates_org = Particular.unscoped.where(particular_status: 1, ledger_id: ledger.id).where("transaction_date between '2016-07-16' and '2016-09-14'").order(:transaction_date).pluck(:transaction_date).uniq
