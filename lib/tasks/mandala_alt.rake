namespace :mandala_alt do
  ...
  def delete_mandala_data
    ...
    bill_detail_ids = Mandala::BillDetail.joins('INNER JOIN bill  ON bill.bill_no = bill_detail.bill_no').where(bill: {id: bill_ids}).pluck(:id).uniq
    ...
    Mandala::VoucherDetail.joins('INNER JOIN voucher  ON voucher_detail.voucher_no = voucher.voucher_no and voucher_detail.voucher_code = voucher.voucher_code').where(voucher: {id: voucher_ids}).delete_all
    Mandala::Ledger.joins('INNER JOIN voucher  ON ledger.voucher_no = voucher.voucher_no and ledger.voucher_code = voucher.voucher_code').where(voucher: {id: voucher_ids}).delete_all

    receipt_payment_ids = Mandala::ReceiptPaymentSlip.joins('INNER JOIN voucher  ON receipt_payment_slip.voucher_no = voucher.voucher_no and receipt_payment_slip.voucher_code = voucher.voucher_code').where(voucher: {id: voucher_ids}).pluck(:id).uniq
  ...
  task :clear_unwanted_old_data, [:tenant] => 'mandala:validate_tenant' do |task, args|
    ...
    puts "Bills  #{BillVoucherAssociation.where(bill_id: bill_ids).count}"
  ...
  task :sync_data, [:tenant] => :environment do |task, args|
    if args.tenant.present?
      ...
      bench = Benchmark.measure do
        Rake::Task["mandala:sync_vouchers"].invoke(tenant,'2073/2074')
        Rake::Task["mandala:sync_bills"].invoke(tenant,'2073/2074')
