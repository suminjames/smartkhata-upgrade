  ...
  def create
    ...
    ids_with_email.each do |id|
      ...
      ActiveRecord::Base.transaction do
        User.invite!(:email => account.email) if valid_email?(account.email)
        accounts.each do |a|
    ...
    ids_without_email.each do |id|
      ...
      ActiveRecord::Base.transaction do
        new_user = User.create!({:username => get_user_name_from_boid(account.boid), :role => :user, :password => get_user_name_from_boid(account.boid), :password_confirmation => get_user_name_from_boid(account.boid), confirmed_at: Time.now, email: nil })
