class ShareTransactionsController < ApplicationController
  before_action :set_share_transaction, only: %i[show edit update destroy process_closeout available_balancing_transactions]

  before_action -> {authorize @share_transaction}, only: %i[show edit update destroy]
  before_action -> {authorize ShareTransaction}, only: %i[index new create deal_cancel pending_deal_cancel capital_gain_report threshold_transactions contract_note_details securities_flow closeouts make_closeouts_processed process_closeout available_balancing_transactions sebo_report commission_report]
  ...
  def index
    ...
    if params[:paginate] == 'false'
      if %w[xlsx pdf].include?(params[:format])
        @share_transactions = if params[:group_by_company] == "true"
                                @filterrific.find.includes(:isin_info, :bill, :client_account).order('isin_info_id ASC, share_transactions.date ASC, contract_no ASC')
                              else
                                @filterrific.find.includes(:isin_info, :bill, :client_account).order('share_transactions.date ASC, contract_no ASC')
                              end
      else
    ...
    params[:filterrific][:group_by_company] = params[:group_by_company] if params[:filterrific] && params[:group_by_company].present?
  ...
  def securities_flow
    ...
    items_per_page = @securities_flows.size if params[:paginate] == 'false'
  ...
  def capital_gain_report
    ...
    items_per_page = 20
    items_per_page = @share_transactions.size if params[:paginate] == 'false'
  ...
  def make_closeouts_processed
    ...
    closeout_settled = params[:make_processed] == 'true'
