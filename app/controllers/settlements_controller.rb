class SettlementsController < ApplicationController
  ...
  def index
    ...
    if ['xlsx', 'pdf'].include?(params[:format])
      @settlements = @filterrific.find.not_rejected.includes(voucher: :cheque_entries)
    else
      @settlements = @filterrific.find.not_rejected.includes(voucher: :cheque_entries).page(params[:page]).per(items_per_page)
    end
