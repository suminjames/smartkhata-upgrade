class SettlementsController < ApplicationController
  ...
  def index
    ...
    if ['xlsx', 'pdf'].include?(params[:format])
      ...
      @settlements = @filterrific.find.not_rejected.includes(:particulars => {:cheque_entries => [{:bank_account => :bank}, :additional_bank]}).order(order_parameter).references(:cheque_entries).page(params[:page]).per(items_per_page).decorate
    end
