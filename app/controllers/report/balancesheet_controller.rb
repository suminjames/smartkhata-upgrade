  ...
  def index
    ...
    @balance.each do |balance|
      if balance.sub_report == Group.sub_reports['Assets']
        @balance_dr[balance.name] = balance.get_ledger_group(drill_level: @selected_drill_level, fy_code: selected_fy_code, branch_id: selected_branch_id)
        @opening_balance_dr += balance.get_ledger_group(fy_code: selected_fy_code, branch_id: selected_branch_id)[:balance]
      end
