class EsewaPaymentsController < VisitorsController
  ...
  before_action :set_esewa_payment, only: [:failure, :success]
  ...
  def create
    ActiveRecord::Base.transaction do
      ...
      if @esewa_payment.save
        ...
        @esewa_payment.update(failure_url: get_failure_url + "&oid=#{@esewa_payment.get_transaction_id}")

        if receipt_transaction.persisted?
          render json: { payment: @esewa_payment, product_id: @esewa_payment.get_transaction_id, security_code: get_esewa_security_code }
        else
  ...
  def set_esewa_payment
    ...
    @esewa_payment      = @receipt_transaction.receivable
  end
