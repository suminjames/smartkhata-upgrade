class ClientAccountsController < ApplicationController
  before_action :set_client_account, only: %i[show edit update destroy]

  before_action -> {authorize @client_account}, only: %i[show edit update destroy]
  before_action -> {authorize ClientAccount}, only: %i[index new create combobox_ajax_filter]
  ...
  def index
    ...
    params[:filterrific] ||= {}
    params[:filterrific][:by_selected_session_branch_id] = selected_branch_id
    ...
    items_per_page = params[:paginate] == 'false' || %w[xlsx pdf].include?(params[:format]) ? ClientAccount.all.count : 20
    @client_accounts = if params[:paginate] == 'false'
                         @filterrific.find.order(:name)
                       else
                         @filterrific.find.order(:name).page(params[:page]).per(items_per_page)
                       end
    @download_path_xlsx = client_accounts_path(params.permit(:format).merge({format: 'xlsx'}))
    @download_path_pdf = client_accounts_path(params.permit(:format).merge({format: 'pdf'}))
  ...
  def combobox_ajax_filter
    ...
    client_accounts = ClientAccount.find_similar_to_term(search_term, selected_session_branch_id) if search_term && search_term.length >= 3
    respond_to do |format|
