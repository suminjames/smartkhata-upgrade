class InterestParticularsController < ApplicationController
  ...
  def index
    ...
    @filterrific = initialize_filterrific(
      InterestParticular.with_client,
      params[:filterrific],
      select_options: {
    ...
    @interest_particulars = @filterrific.find.order(date: :asc).page(params[:page]).per(20)

    if params[:filterrific].present?
      @total_dr = @filterrific.find.dr.sum(:interest)
      @total_cr = @filterrific.find.cr.sum(:interest)
    end
