class LedgersController < ApplicationController
  ...
  def show
    ...
    if ledger_query.error_message.present?
      respond_to do |format|
  ...
  def show_all
    ...
    @particulars = Particular.includes(:voucher).where(date_bs: @date_bs).where('vouchers.voucher_type IN (1,2,4,5,6,7,8)').where('vouchers.created_at <= ?', '2016-11-01').references(:vouchers).order(ledger_id: :desc) if params[:date_bs].present?
  end
  ...
  def merge_ledger
    ...
    if (merge_to && merge_from).present?
      ...
      if merge_bool
        flash[:notice] = "Sucessfully Ledger Merge"
      else
  ...
  def combobox_ajax_filter
    ...
    if search_term && search_term.length >= 3
      ledgers = if user_has_access_to?(restricted_ledgers_path)
                  params[:restricted] == 'true' ? Ledger.restricted : Ledger
                else
                  Ledger.unrestricted
                end
  ...
  def group_members_ledgers
    if params[:client_account_id]
      ...
      @ledgers = @client_account&.get_group_members_ledgers_with_balance || []
    end
  ...
  def transfer_group_member_balance
    ...
    redirect_to @back_path, flash: {error: 'Invalid Ledgers'} and return unless (@ledger_ids - group_member_ledger_ids).empty?
    ...
    ActiveRecord::Base.transaction do
      ...
      voucher = Voucher.create!(current_user_id: current_user.id, branch_id: selected_branch_id, fy_code: selected_fy_code, date_bs: ad_to_bs_string(Time.zone.now), desc: description, voucher_status: :complete)
      ...
      ledger_list.each do |ledger|
        ...
        if ledger.closing_balance(get_fy_code).abs > 0.01
          ...
          process_accounts(ledger, voucher, _closing_balance.negative?, _closing_balance.abs, description, selected_branch_id, Time.zone.now.to_date, current_user)
        end
      end

      process_accounts(group_leader_ledger, voucher, net_balance >= 0, net_balance.abs, description, selected_branch_id, Time.zone.now.to_date, current_user)
      raise ActiveRecord::Rollback if net_balance == 0.0
    end

    # also redirect to same path
    redirect_to @back_path, flash: {error: 'The balance to transfer is 0'} and return if net_balance == 0.0
  ...
  def ledger_params
    permitted_params = params.require(:ledger).permit(:name, :opening_blnc, :group_id, :opening_balance_type, :vendor_account_id, ledger_balances_attributes: %i[opening_balance opening_balance_type branch_id fy_code current_user_id id])
