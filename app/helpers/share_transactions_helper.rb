module ShareTransactionsHelper
  ...
  def report_headings_for_securities_flow(params, is_securities_balance_view)
    ...
    report_headings = []
    report_headings << if is_securities_balance_view
                         "Securities Balance"
                       else
                         "Securities Inwards/Outwards Register"
                       end
    report_headings << "Company: #{IsinInfo.find(isin_info_id).name_and_code}" if isin_info_id.present?
