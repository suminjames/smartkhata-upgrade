...
class LedgerDaily < ActiveRecord::Base
  ...
  def self.sum_of_closing_balance_of_ledger_dailies_for_ledgers(ledger_ids, date_to_ad, selected_fy_code, selected_branch_id)
    ...
    Ledger.where(id:ledger_ids).each do |ledger|
      last_day_ledger_daily = ledger.ledger_dailies.by_branch_fy_code(selected_fy_code, selected_branch_id).where('date <= ?',date_to_ad).order('date DESC, updated_at DESC').first
