...
class BankAccount < ActiveRecord::Base
  ...
  def change_default
    if self.default_for_payment
      bank_accounts = BankAccount.by_branch_id(selected_branch).where(:default_for_payment => true)
      bank_accounts = BankAccount.by_branch_id(selected_branch).where.not(:id => self.id)
      bank_accounts.update_all(:default_for_payment => false)
    end

    if self.default_for_receipt
      bank_accounts = BankAccount.by_branch_id(selected_branch).where(:default_for_receipt => true)
      bank_accounts = BankAccount.by_branch_id(selected_branch).where.not(:id => self.id)
  ...
  def self.default_for_payment
    BankAccount.by_branch_id(selected_branch).where(:default_for_payment => true).first
  end

  def self.default_receipt_account
    default_for_receipt_bank_account_in_branch = BankAccount.by_branch_id(selected_branch).where(:default_for_receipt => true).first
