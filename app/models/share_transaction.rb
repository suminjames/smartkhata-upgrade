...
class ShareTransaction < ApplicationRecord
  ...
  enum transaction_type: { buying: 0, selling: 1, unknown: 2 }
  enum transaction_cancel_status: { no_deal_cancel: 0, deal_cancel_pending: 1, deal_cancel_complete: 2 }
  ...
  scope :find_by_date_range, lambda { |date_from, date_to|
    where(
  ...
  scope :by_transaction_cancel_status, lambda { |status|
    where(transaction_cancel_status: ShareTransaction.transaction_cancel_statuses[status]) if status == 'deal_cancel_complete'
  }
  ...
  scope :by_other_broker_number_closeouts, lambda { |number|
  ...
  scope :settled_with_bill, -> { where.not(bill_id: nil) }
  scope :settled, -> { where.not(settlement_id: nil) }
  scope :cgt_gt_zero, -> { where('cgt > ?', 0.0) }

  scope :for_cgt, lambda { |fy_code|
  ...
  scope :above_threshold, -> { not_cancelled.where("net_amount >= ?", 1_000_000) }
  ...
  def self.securities_flows(tenant_broker_id, isin_id, date_bs, date_from_bs, date_to_bs, branch_id)
    ...
    where_condition_str = if where_conditions.present?
                            "WHERE #{where_conditions.join(" AND ")} AND client_accounts.branch_id = #{branch_id}"
                          else
                            "WHERE client_accounts.branch_id = #{branch_id}"
                          end
  ...
  def self.sebo_report(isin_id, date_from_bs, date_to_bs, branch_id, selected_fy_code)
    ...
    where_conditions << "(branch_id = #{branch_id})" if branch_id != 0
  ...
  def unique_types
    grouped_types.uniq.sort.map { |x| ShareTransaction.transaction_types.keys[x].titleize }.join(', ') if grouped_types
  end
  ...
  def self.commission_report(client_id, date_from_bs, date_to_bs, selected_fy_code)
    ...
    ShareTransaction.includes(:client_account).where(where_condition_str).group(:client_account_id).select(
      ...
      "SUM(raw_quantity) as total_quantity",
      "SUM(share_amount) as total_amount",
      "SUM(commission_amount-nepse_commission) as total_commission_amount"
  ...
  def self.options_for_transaction_type_select
    ...
    #     ['Selling', 'selling'],
    # ]
    ShareTransaction.transaction_types.except(:unknown).map { |v, _i| [v.titleize, v] }
  end
  ...
  def calculate_base_price
    ...
    if buying? || settlement_id.blank? || quantity == 0 || purchase_price == 0
      ...
      commission_rates_desc_snipped.reverse_each do |commission_rate|
        possible_base_price = if commission_rate.to_s.include?('flat_')
                                purchase_price - commission_rate.split("_")[1].to_f
                              else
                                100.0 * purchase_price / (commission_rate + 100.0)
                              end
        ...
        next unless commission_rate == commission_rate_for_possible_share_amount
        ...
        calculated_base_price /= quantity
        break
      end
    end
