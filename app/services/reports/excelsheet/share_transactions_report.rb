class Reports::Excelsheet::ShareTransactionsReport
  ...
  def generate_excelsheet
    ...
    workbook.add_worksheet(name: "Sheet 1") do |sheet|
      ...
      workbook.styles do |s|
        ...
        # Fixed width for date which may be elongated by document headers
  ...
  def add_styles(obj)
    ...
    doc_header = obj.add_style sz: 20, fg_color: "3c8dbc", bg_color: "FF"
    doc_sub_header = obj.add_style sz: 14, bg_color: "FF"
    blank = obj.add_style bg_color: "FF"
  ...
  def setup_with_document_headers
    ...
    if @params[:search_by] == 'client' && client_account = ClientAccount.find_by(id: @params[:search_term])
      add_document_headers("Client Wise Report", "\"#{client_account.name.strip}\"")
    ...
    # search by company
      ...
      add_document_headers("Company Wise Report", "\"#{isin.company.strip}\"")
      ...
      add_document_headers("Share Inventory Report", "All transactions")
  ...
  def add_document_headers(title, sub_title)
    ...
    @sheet.add_row [""], style: @styles[:blank]
