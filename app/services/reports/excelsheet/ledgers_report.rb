class Reports::Excelsheet::LedgersReport < Reports::Excelsheet
  ...
  TABLE_HEADER = ["Date", "Date AD", "Particulars", "Voucher", "Bill", "Cheque", "Pay/Receipt No", "Transaction Amount", "", "Balance"].freeze
  ...
  def add_document_headings(heading, sub_heading, opening_closing_blnc)
    ...
    add_document_headings_base(heading, sub_heading, opening_closing_blnc) do
      # if date queries present
      if @params && @params[:search_by] == "date_range" && @params[:search_term].present? && %i[date_from date_to].any? { |x| @params[:search_term][x].present? }
        date_from = @params[:search_term][:date_from].presence || '*'
        date_to = @params[:search_term][:date_to].presence || '*'
  ...
  def populate_data_rows
    ...
    particulars_ids.in_groups_of(1000).each do |photo_ids|
      query.where(id: photo_ids).each_with_index do |p, index|
        ...
        bills = ""
        p.bills.each_with_index do |bill, _bill_index|
        ...
        settlements = p.settlements.map { |settlement| settlement.id.to_s }.join(", ")
        ...
        balance << ((running_total + margin_of_error_amount).negative? ? " cr" : " dr")
