class Reports::Pdf::SeboReport < Prawn::Document
  ...
  def securities_flows_list
    ...
    table(table_data, header: true) do |t|
      ...
      t.row(-1).columns(1..20).borders = %i[bottom top]
      t.row(-1).columns(0).borders = %i[bottom left top]
      t.row(-1).columns(21).borders = %i[right bottom top]
  ...
  def company_header
    ...
    bounding_box([0, row_cursor], width: col(9)) do
      ...
      elsif text "<b>Fiscal Year #{@fiscal_year}</b>", inline_format: true, size: 10, align: :center
      end
    end
    hr
  end
