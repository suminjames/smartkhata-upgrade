class Reports::Pdf::ShareTransactionsCloseoutReport < Prawn::Document
  ...
  def self.file_name(params)
    ...
    if @client_account && @isin_info
      @file_name = "ClientCompany_ShareTransactionCloseoutReport_#{@client_account.nepse_code}_#{@isin_info.isin}_"
    elsif @client_account
      @file_name = "ClientWise_ShareTransactionCloseoutReport_#{@client_account.nepse_code}"
    elsif @isin_info
      @file_name = "CompanyWise_ShareTransactionCloseoutReport_#{@isin_info.isin}"
    else # full report
      @file_name = "ShareTransactionCloseoutReport_#{Date.today}"
    end
  end
