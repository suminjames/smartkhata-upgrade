    ...
    unless @error_message
      ActiveRecord::Base.transaction do 
        @processed_data.each do |hash|
          ...
          @date_set.add(Date.parse(hash['ORDER_TIME'].to_s))

          # Each order_id listed in the excel sheet should be checked for duplicate entry in the database.
          #if is_record_available_in_db(order_id)
            #import_error("File upload cancelled! An order with order id " + order_id.to_s + " seems to have been previously uploaded! Please double check and upload.")
          ...
          order_obj                   = Order.new()
          order_obj.order_id          = hash['ORDER_ID']
          order_obj.isin_info_id      = isin_info_id
          order_obj.client_account_id = client_account_id
          order_obj.price             = hash['PRICE']
          order_obj.quantity          = hash['QUANTITY']
          order_obj.amount            = hash['AMOUNT']
          order_obj.pending_quantity  = hash['PENDING_QUANTITY']
          order_obj.order_date_time   = Time.parse(hash['ORDER_DATE_TIME'].to_s)
          order_obj.order_type        = hash['ORDER_TYPE'].downcase
          order_obj.order_segment     = hash['ORDER_SEGMENT']
          order_obj.order_condition   = hash['ORDER_CONDITION']
          order_obj.order_state       = hash['ORDER_STATE'].downcase
          order_obj.save!
        end
