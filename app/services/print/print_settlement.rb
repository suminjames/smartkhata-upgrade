class Print::PrintSettlement < Prawn::Document
    ...
    if @settlement.cash_amount.blank?
      ...
      if @settlement.cheque_entries.size > 0
        ...
        _particulars = if @settlement.receipt? || @settlement.belongs_to_batch_payment?
                         @settlement.debited_particulars.uniq
                       else
                         @settlement.credited_particulars.uniq
                       end
        ...
        _particulars.each do |p|
          p.cheque_entries.uniq.each do |cheque|
            ...
            cheque_amount = if cheque.amount >= p.amount
                              arabic_number(p.amount)
                            else
                              arabic_number(cheque.amount)
                            end
  ...
  def footer
    ...
    text '<u><i>Note: Please bring this receipt compulsarily while claiming unpurchase share.</i></u>', inline_format: true if @settlement.receipt?
  end
