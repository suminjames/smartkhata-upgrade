class Print::PrintMultipleBills < Prawn::Document
  ...
  def initialize(bills, current_tenant, print_type)
    ...
    bills.each_with_index do |bill, index|
      @bill = bill
      draw
      start_new_page if index != bills.length - 1
    end
  end
  ...
  def calculation_section
    ...
    bounding_box([0, row_cursor], width: col(6)) do
      ...
      if @bill.bill_type == 'sales'
        tr_data.delete_at(5)
        tr_data.insert(5, ["Closeout Deduction:", @bill.formatted_net_closeout_amount]) if @bill.closeout_charge.positive?
      end
  ...
  def share_transactions_section
    ...
    bounding_box([0, row_cursor], width: col(12)) do
      ...
      column_widths = if @bill.bill_type == 'purchase'
                        {0 => table_width * 0.26,
                         1 => table_width * 0.20,
                         2 => table_width * 0.09,
                         3 => table_width * 0.09,
