...
(function($){
	$(document).ready(function(){
		var sum_total = 0;
		$('#isin_holdings .total').each(function(){
			...
			$parent.find('.total').html(sum.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,"));

			var price = Number($parent.find('.price').text());
			if(price > 0 )
			{
				$parent.find('.total_value').html((sum*price).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,"));
				sum_total += (sum * price)
			} else {
				$parent.find('.total_value').html('No Data');
			}

		});
		$('.sum_total').html(sum_total.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,"));
	});
})(jQuery);